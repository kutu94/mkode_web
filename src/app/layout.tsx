          ...
          <header className="fixed top-0 z-50 w-full glass-nav">
            <nav className="container mx-auto px-4 sm:px-6" aria-label="Navegación principal">
              <div className="flex h-16 items-center justify-between">
                <Link href="/" className="group flex items-center gap-3 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300" aria-label="MKode, inicio">
                ...
                <div className="hidden items-center gap-8 md:flex">
                  {navigationItems.map(item => item.cta ? (
                    <Link key={item.href} href={item.href} className="rounded-2xl px-5 py-2 text-xs font-bold uppercase tracking-widest btn-gradient soft-shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300">{item.label}</Link>
              ...
              <div className="grid h-12 grid-cols-4 items-stretch gap-1 border-t border-white/10 md:hidden" aria-label="Navegación móvil">
                {navigationItems.map(item => <MobileLink key={item.href} href={item.href}>{item.label}</MobileLink>)}
              </div>
            </nav>
          </header>
            ...
            <div className="container mx-auto px-6">
              <div className="mb-12 grid grid-cols-1 gap-10 md:mb-16 md:grid-cols-4 md:gap-12">
                ...
                <div>
                  ...
                  <ul className="space-y-4">{navigationItems.map(item => <li key={item.href}><Link href={item.href} className="text-sm text-white/70 hover:text-white">{item.label}</Link></li>)}</ul>
                </div>
