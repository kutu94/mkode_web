...
const LogoIcon = ({ className }: { className?: string }) => (
  ...
  >
    ...
    />
    <path
      d="M10 50 H30 M70 50 H90 M25 40 L10 40 M75 60 L90 60"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
    />
        ...
        <div className="flex min-h-screen flex-col">
          <header className="fixed top-0 z-50 w-full glass-nav">
            ...
            >
                  ...
                  {navigationItems.map((item) =>
                    item.cta ? (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="rounded-2xl px-5 py-2 text-xs font-bold uppercase tracking-widest btn-gradient soft-shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300"
                      >
                        {item.label}
                      </Link>
                    ) : (
              ...
              <div
                className="grid h-12 grid-cols-4 items-stretch gap-1 border-t border-white/10 md:hidden"
            ...
            <div className="container mx-auto px-6">
              <div className="mb-12 grid grid-cols-1 gap-10 md:mb-16 md:grid-cols-4 md:gap-12">
                <div className="col-span-1 md:col-span-2">
                  ...
                  <p className="max-w-sm text-sm leading-relaxed text-white/70">
                    Ingeniería de sistemas inteligentes para optimizar operaciones de empresas en
                    crecimiento.
                  </p>
                </div>
                <div>
                  <h2 className="mb-6 text-xs font-bold uppercase tracking-widest text-white">
                    Secciones
                  </h2>
              ...
              <div className="flex flex-col items-center justify-between gap-4 border-t border-white/5 pt-8 md:flex-row">
                <p className="text-[11px] uppercase tracking-widest text-white/60">
                  © {new Date().getFullYear()} MKode LLC. Engineering for operations.
                </p>
                <p className="text-[11px] uppercase tracking-widest text-white/60 italic">
                  Serious Technical Excellence
                </p>
              </div>
            </div>
          </footer>
...
function MobileLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="flex min-w-0 items-center justify-center rounded-md px-1 text-center text-[10px] font-semibold uppercase leading-tight tracking-wide text-white/80 hover:bg-white/5 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-300"
    >
      {children}
    </Link>
  );
